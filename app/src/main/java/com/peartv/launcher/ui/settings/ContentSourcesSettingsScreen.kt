...
/**
 * §4 sub-page (`design/settings-menu.png` rework) — the two settings that
 * feed the hero's own content tiers (§2.4/§3.1.1): the TMDB API key (Tier
 * 1 backdrop art) and the Home Screen Channels permission CTA (Tier 3).
 * Grouped together as "where the app's richer content comes from," same
 * reasoning both had for living on one flat page before this rework.
 */
@Composable
fun ContentSourcesSettingsScreen(
    ...
    SettingsPageScaffold(title = "Content Sources", modifier = modifier) {
        Column(modifier = Modifier.width(SettingsCategoryListWidth)) {
            ...
            BasicTextField(
                ...
                modifier = Modifier
                    .focusRequester(keyFieldFocusRequester)
    ...
    LaunchedEffect(Unit) {
        keyFieldFocusRequester.requestFocus()
    }
}
